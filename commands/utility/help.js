  ...
  execute(message, args) {
    ...
    if (!args.length) {
      ...
      return message.author.send(data, {
        ...
        message.reply(
          'Aku telah mengirimkan DM dengan semua perintah untuk aku');
      }).catch(error => {
        console.error(
          `Aku tidak dapat mengirimkan bantuan ke ${message.author.tag}.\n`,
          error);
        message.reply(
          'Aku tidak bisa DM kamu! Apakah kamu menonaktifkan DM?');
      })
    }

    const name = args[0].toLowerCase();
    const command = commands.get(name) || commands.find(c => c.aliases && c
